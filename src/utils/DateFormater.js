...
const FORMATE_2 = 'yyyy-MM-dd hh:mm:ss';

function DateFormater (timestamp) {
    // timestamp = parseInt(timestamp);
    if ( !this instanceof DateFormater ) return new DateFormater(timestamp);
    
    this._d = new Date(timestamp);
}

DateFormater.prototype.format = function (dateString) {
    dateString = dateString.replace(/h/g, 'H');
    const date = {
...
DateFormater.prototype.get_hours = function () {
    const hours = this._d.getHours();
    return (hours > 12) ? hours - 12 : hours;
};
export default function convertTimestampToDate (value) {
    return (new DateFormater(value)).format(FORMATE_2);
}
